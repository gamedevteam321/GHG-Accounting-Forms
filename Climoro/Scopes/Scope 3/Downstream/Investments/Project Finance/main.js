(function(){
  ...
  function applyDateFilter(records) {
    ...
    console.log('Project Finance: Applying client-side date filtering...');
  ...
  function loadExisting(){
    const specs = [
      { tbody: tbSpecific, doctype: 'Downstream Project Finance Project Specific Item', fields: ['name','s_no','date','company','share','scope1','scope2','co2e'] },
      { tbody: tbAverage, doctype: 'Downstream Project Finance Average Data Item', fields: ['name','s_no','date','company','share','cost','currency','eeio_ef','ef_unit','co2e'] },
    ];
      ...
      frappe.call({ 
        ...
        callback: r => {
          if (r.message && r.message.length > 0) {
            ...
            console.log(`Project Finance: Original records: ${r.message.length}, Filtered records: ${filteredRecords.length}`);
  ...
  function buildFilterBar(done){ 
    ...
    bar.querySelector('.filter-apply-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Project Finance Filter values:', {
    ...
    bar.querySelector('.filter-clear-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Project Finance Date filters cleared');
      [tbSpecific, tbAverage].forEach(tb => tb?.querySelectorAll('.data-row').forEach(r=>r.remove()));
      loadExisting();
    });
