(function(){
  ...
  function applyDateFilter(records) {
    ...
    console.log('Debt Investments: Applying client-side date filtering...');
  ...
  function loadExisting(){
    const specs = [
      { tbody: tbSpecific, doctype: 'Downstream Debt Investments Project Specific Item', fields: ['name','s_no','date','project_id','scope1','scope2','share','co2e'] },
      { tbody: tbAverage, doctype: 'Downstream Debt Investments Average Data Item', fields: ['name','s_no','date','project_type','cost','eeio_ef','unit','share','co2e'] },
    ];
      ...
      frappe.call({
        ...
        callback: r => {
          if (r.message && r.message.length > 0) {
            ...
            console.log(`Debt Investments: Original records: ${r.message.length}, Filtered records: ${filteredRecords.length}`);
  ...
  function buildFilterBar(done){ 
    ...
    bar.querySelector('.filter-apply-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Debt Investments Filter values:', {
    ...
    bar.querySelector('.filter-clear-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Debt Investments Date filters cleared');
      [tbSpecific, tbAverage].forEach(tb => tb?.querySelectorAll('.data-row').forEach(r=>r.remove()));
      loadExisting();
    });
