(function(){
  ...
  function applyDateFilter(records) {
    ...
    console.log('Total Projected Emissions: Applying client-side date filtering...');
  ...
  function loadExisting(){
    (async ()=>{
      ...
      frappe.call({ 
        ...
        args:{ doctype, fields: ['name','s_no','date','project_id','share','annual','lifetime','co2e'], limit_page_length: 500, order_by: 'creation asc', filters }, 
        callback: r => {
          if (r.message && r.message.length > 0) {
            // Apply client-side date filtering
            const filteredRecords = applyDateFilter(r.message);
            console.log(`Total Projected Emissions: Original records: ${r.message.length}, Filtered records: ${filteredRecords.length}`);
  ...
  function buildFilterBar(done){ 
    ...
    bar.querySelector('.filter-apply-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Total Projected Emissions Filter values:', {
    ...
    bar.querySelector('.filter-clear-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Total Projected Emissions Date filters cleared');
      tbody.querySelectorAll('.data-row').forEach(r=>r.remove());
      loadExisting();
    });
