(function(){
  ...
  function applyDateFilter(records) {
    ...
    console.log('Downstream Distribution: Applying client-side date filtering...');
      ...
      frappe.call({
        ...
        callback: r => {
          if (r.message && r.message.length > 0) {
            ...
            console.log(`Downstream Distribution: Original records: ${r.message.length}, Filtered records: ${filteredRecords.length}`);
  ...
  function buildFilterBar(done){ 
    ...
    bar.querySelector('.filter-apply-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Downstream Distribution Filter values:', {
      ...
      [tbSite, tbAvg].forEach(tb => tb?.querySelectorAll('.data-row').forEach(r=>r.remove()));
      loadExisting();
    });
    ...
    bar.querySelector('.filter-clear-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Downstream Distribution Date filters cleared');
      [tbSite, tbAvg].forEach(tb => tb?.querySelectorAll('.data-row').forEach(r=>r.remove()));
      loadExisting();
    });
