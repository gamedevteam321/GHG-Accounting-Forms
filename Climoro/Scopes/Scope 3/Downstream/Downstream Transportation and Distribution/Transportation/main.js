(function(){
  ...
  function applyDateFilter(records) {
    ...
    console.log('Downstream Transportation: Applying client-side date filtering...');
      ...
      frappe.call({
        ...
        callback: r => {
          if (r.message && r.message.length > 0) {
            ...
            console.log(`Downstream Transportation: Original records: ${r.message.length}, Filtered records: ${filteredRecords.length}`);
  ...
  function buildFilterBar(done){ 
    ...
    bar.querySelector('.filter-apply-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Downstream Transportation Filter values:', {
      ...
      [tbFuel, tbDistance, tbSpend].forEach(tb => tb?.querySelectorAll('.data-row').forEach(r=>r.remove()));
      loadExisting();
    });
    ...
    bar.querySelector('.filter-clear-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Downstream Transportation Date filters cleared');
      [tbFuel, tbDistance, tbSpend].forEach(tb => tb?.querySelectorAll('.data-row').forEach(r=>r.remove()));
      loadExisting();
    });
