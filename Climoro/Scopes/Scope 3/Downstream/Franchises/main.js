(function(){
  ...
  function addFromEntry(entry){
    ...
    let map;
    if (tId === 'fran-tb-specific') map = { doctype: 'Downstream Franchises Franchise Specific Item' };
    else if (tId === 'fran-tb-average') map = { doctype: 'Downstream Franchises Average Data Item' };
    if (!map) return;

    const data = {};
    entry.querySelectorAll('input,select').forEach(inp=>{
      const key = inp.name === 'franchise_id' ? 'franchise_id' : (inp.name === 'type' ? 'franchise_type' : inp.name);
      data[key] = inp.value;
    });
  ...
  function applyDateFilter(records) {
    ...
    console.log('Franchises: Applying client-side date filtering...');
  ...
  function loadExisting(){
    const specs = [
      { tbody: tbSpecific, doctype: 'Downstream Franchises Franchise Specific Item', fields: ['name','s_no','date','franchise_id','scope1','scope2','co2e'] },
      { tbody: tbAverage, doctype: 'Downstream Franchises Average Data Item', fields: ['name','s_no','date','franchise_type','count','avg','unit','co2e'] },
    ];
      ...
      frappe.call({
        ...
        callback: r => {
          if (r.message && r.message.length > 0) {
            ...
            console.log(`Franchises: Original records: ${r.message.length}, Filtered records: ${filteredRecords.length}`);
  ...
  function buildFilterBar(done){ 
    ...
    bar.querySelector('.filter-apply-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Franchises Filter values:', {
    ...
    bar.querySelector('.filter-clear-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Franchises Date filters cleared');
      [tbSpecific, tbAverage].forEach(tb => tb?.querySelectorAll('.data-row').forEach(r=>r.remove()));
      loadExisting();
    });
