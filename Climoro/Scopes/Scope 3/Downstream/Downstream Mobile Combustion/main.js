// Downstream Mobile Combustion Custom Block JavaScript for Frappe (Separate DocTypes)
(function() {
    // Global variables
    ...
    function initializeTable() {
        ...
        console.log('Initializing downstream mobile combustion...');
        ...
        Promise.all([getUserRegion(), loadFuelTypes(), loadTransportationTypes()]).then(() => {
            ...
            isInitialized = true;
            console.log('Downstream mobile combustion initialized successfully with region:', userRegion, 'and fuel types:', fuelSelections);
        });
    }
    ...
    function addNewFuelRow(entryRow) {
        ...
        // Save to Downstream Fuel Method doctype
        saveToDoctype('Downstream Fuel Method', formData, (success, docName) => {
            if (success) {
                // Create display row
    ...
    function addNewTransportationRow(entryRow) {
        ...
        // Save to Downstream Transportation Method doctype
        saveToDoctype('Downstream Transportation Method', formData, (success, docName) => {
            if (success) {
                // Create display row
    ...
    function getFuelFormData(row) {
        // Clean vehicle number to remove any special characters that might cause naming issues
        const vehicleNo = row.querySelector('.vehicle-no-input').value.replace(/[<>]/g, '');
        
        return {
            naming_series: 'MCFM-.YYYY.-',
    ...
    function saveToDoctype(doctypeName, data, callback) {
        ...
        // Clean any potential special characters from vehicle number
        data.vehicle_no = data.vehicle_no.replace(/[<>]/g, '').trim();
        
        // Ensure naming series exists
        const namingSeries = data.naming_series;
        if (namingSeries) {
            frappe.call({
                method: 'frappe.client.get_value',
                args: {
                    doctype: 'Naming Series',
                    filters: { name: namingSeries },
                    fieldname: 'name'
                },
                callback: function(namingResult) {
                    if (!namingResult.message) {
                        // Create the naming series if it doesn't exist
                        frappe.call({
                            ...
                            callback: function(createResult) {
                                if (createResult.exc) {
                                    console.log('Naming series creation failed, continuing with save:', createResult.exc);
                                }
                                proceedWithSave();
                            }
                        });
        ...
        function proceedWithSave() {
            frappe.call({
                ...
                callback: function(r) {
                    if (r.exc) {
                        console.error('Error saving to', doctypeName, ':', r.exc);
                        let errorMessage = r.exc;
                        
                        // Provide more specific error messages
                        if (r.exc.includes('Name cannot contain special characters')) {
                            errorMessage = 'Vehicle number contains invalid characters. Please use only letters, numbers, and common symbols.';
                        } else if (r.exc.includes('Duplicate entry')) {
                            errorMessage = 'A record with this vehicle number already exists.';
                        } else if (r.exc.includes('naming series')) {
                            errorMessage = 'Naming series configuration error. Please contact administrator.';
                        }
                        
                        showNotification('Error saving data: ' + errorMessage, 'error');
                        callback(false, null);
                    } else {
                        console.log('Successfully saved to', doctypeName, ':', r.message.name);
    ...
    function createFuelDisplayRow(data, docName) {
        ...
        displayRow.innerHTML = `
            ...
            <td>${data.etco2eq || ''}</td>
            <td>
                <button type="button" class="btn btn-danger btn-sm delete-row-btn" data-doc-name="${docName}" data-doctype="Downstream Fuel Method">
    ...
    function createTransportationDisplayRow(data, docName) {
        ...
        displayRow.innerHTML = `
            ...
            <td>${data.etco2eq || ''}</td>
            <td>
                <button type="button" class="btn btn-danger btn-sm delete-row-btn" data-doc-name="${docName}" data-doctype="Downstream Transportation Method">
                    <i class="fa fa-trash"></i>
                </button>
            </td>
        `;
    ...
    function loadExistingData() {
        // Load existing fuel method data
        frappe.call({
            method: 'frappe.client.get_list',
            args: {
                doctype: 'Downstream Fuel Method',
                fields: ['name', 's_no', 'date', 'vehicle_no', 'fuel_selection', 'fuel_used', 'unit_selection', 
            ...
            callback: function(r) {
                if (r.message && r.message.length > 0) {
                    savedFuelData = r.message;
                    console.log('Loaded existing fuel data:', savedFuelData);
                    
                    // Create display rows for existing data
                    savedFuelData.forEach(data => {
                        createFuelDisplayRow(data, data.name);
                    });
                    
                    // Update current row ID
                    if (savedFuelData.length > 0) {
                        currentFuelRowId = Math.max(...savedFuelData.map(item => item.s_no || 0)) + 1;
                    }
                }
            }
        });

        // Load existing transportation method data
        frappe.call({
            method: 'frappe.client.get_list',
            args: {
                doctype: 'Downstream Transportation Method',
                fields: ['name', 's_no', 'date', 'vehicle_no', 'transportation_type', 'distance_traveled', 'unit_selection',
                        'efco2', 'efch4', 'efn20', 'eco2', 'ech4', 'en20', 'etco2eq'],
                order_by: 's_no asc',
                limit: 100
            },
            callback: function(r) {
                if (r.message && r.message.length > 0) {
                    savedTransportData = r.message;
                    console.log('Loaded existing transportation data:', savedTransportData);
                    
                    // Create display rows for existing data
                    savedTransportData.forEach(data => {
                        createTransportationDisplayRow(data, data.name);
                    });
                    
                    // Update current row ID
                    if (savedTransportData.length > 0) {
                        currentTransportRowId = Math.max(...savedTransportData.map(item => item.s_no || 0)) + 1;
                    }
                }
            }
        });
