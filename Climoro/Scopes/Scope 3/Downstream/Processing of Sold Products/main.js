(function () {
  ...
  function applyDateFilter(records) {
    ...
    console.log('Processing of Sold Products: Applying client-side date filtering...');
    ...
    specs.forEach(async spec =>{
      ...
      frappe.call({ method:'frappe.client.get_list', args:{ doctype: spec.doctype, fields: spec.fields, limit_page_length: 500, order_by: 'creation asc', filters }, callback: r => {
        if (r.message && r.message.length > 0) {
          ...
          console.log(`Processing of Sold Products: Original records: ${r.message.length}, Filtered records: ${filteredRecords.length}`);
  ...
  function buildFilterBar(done){ 
    ...
    bar.querySelector('.filter-apply-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Processing of Sold Products Filter values:', {
      ...
      // reload tables
      [tbodySite, tbodyAvg].forEach(tb=> tb?.querySelectorAll('tr.data-row').forEach(r=>r.remove()));
      loadExisting();
    });
    ...
    bar.querySelector('.filter-clear-btn')?.addEventListener('click', ()=>{
      ...
      console.log('Processing of Sold Products Date filters cleared');
      [tbodySite, tbodyAvg].forEach(tb=> tb?.querySelectorAll('tr.data-row').forEach(r=>r.remove()));
      loadExisting();
    });
