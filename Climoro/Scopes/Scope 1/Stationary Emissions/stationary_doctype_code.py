...
#step 3 - Create Stationary Emissions DocType
...
# Delete the existing DocType if it exists
stationary_doctype_name = "Stationary Emissions"
if frappe.db.exists("DocType", stationary_doctype_name):
    frappe.delete_doc("DocType", stationary_doctype_name, force=True)
    frappe.db.commit()
    print("✅ Old Stationary Emissions DocType deleted")

# Create new Stationary Emissions DocType
stationary_doc = frappe.new_doc("DocType")
stationary_doc.name = stationary_doctype_name
stationary_doc.module = "Custom"
stationary_doc.custom = 1
stationary_doc.autoname = "hash"
stationary_doc.title_field = "activity_data"

# Define fields for Stationary Emissions
stationary_fields = [
    # Basic Information
    ...
    {"fieldname": "eco2", "label": "ECO2", "fieldtype": "Float", "read_only": 1, "in_list_view": 1},
    {"fieldname": "ech4", "label": "ECH4", "fieldtype": "Float", "read_only": 1, "in_list_view": 1},
    {"fieldname": "en2o", "label": "EN2O", "fieldtype": "Float", "read_only": 1, "in_list_view": 1},
    {"fieldname": "etco2eq", "label": "ETCO2eq", "fieldtype": "Float", "read_only": 1, "in_list_view": 1}
]

# Add fields to doctype
for field in stationary_fields:
    stationary_doc.append("fields", field)

# Permissions
stationary_doc.append("permissions", {"role": "System Manager", "read": 1, "write": 1, "create": 1, "delete": 1})
stationary_doc.append("permissions", {"role": "All", "read": 1})

# Save doctype
stationary_doc.insert(ignore_permissions=True)
frappe.db.commit()
print("✅ Stationary Emissions DocType created successfully with company and company_unit fields!")
